...
<body>
    ...
    if ($teacher_id > 0) {
        // Query to get teacher's full name, department, status, and break_time_id
    ...
    // Fetch admin's full name for signature
    $adminFullName = '';
    // Fetch the first admin's name (assuming the first admin is the in-charge)
    $query = "SELECT firstName, middleName, lastName FROM admin_account ORDER BY created_at LIMIT 1";
    $stmt = $conn->prepare($query);
    if ($stmt) {
        $stmt->execute();
    ...
    // Get the current month and year from URL or default to current date
        ...
        <nav aria-label="breadcrumb" class="breadcrumbs ms-4 bg-white border shadow rounded py-2 px-3">
            <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item">
                    <a href="teacher-dashboard.php" class="text-muted text-decoration-none">
                        <i class="fa-solid fa-house"></i> Home
                    </a>
                </li>
                <li class="breadcrumb-item active text-muted" aria-current="page">Daily Time Record</li>
            </ol>
        </nav>
    </div>

    <div class="d-flex justify-content-between align-items-center mb-3">
        <p class="classTitle fs-4 fw-bold ms-5 mb-2">Daily Time Record for <?= htmlspecialchars($teacherFullName); ?></p>
    </div>
