...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (
        isset($_POST['teacher_id'], $_POST['day_of_week'], $_POST['subject_code'], $_POST['class_title'], $_POST['time_start'], $_POST['time_end'])
        ...
        $overlap_query = "SELECT * FROM teacher_schedule WHERE teacher_id = ? AND day_of_week = ? AND ((time_start < ? AND time_end > ?) OR (time_start = ? AND time_end = ?))";

        $overlap_stmt = mysqli_prepare($conn, $overlap_query);
        if ($overlap_stmt) {
            mysqli_stmt_bind_param($overlap_stmt, "isssss", $teacher_id, $day_of_week, $time_end, $time_start, $time_start, $time_end);
        ...
        $stmt = mysqli_prepare($conn, "INSERT INTO teacher_schedule (teacher_id, day_of_week, subject_code, class_title, time_start, time_end) VALUES (?, ?, ?, ?, ?, ?)");
        if ($stmt) {
            mysqli_stmt_bind_param($stmt, "isssss", $teacher_id, $day_of_week, $subject_code, $class_title, $time_start, $time_end);
            ...
            if (mysqli_stmt_affected_rows($stmt) > 0) {
                ...
                $_SESSION['message'] = 'Failed to add schedule. Please try again.';
                $_SESSION['message_type'] = 'danger';
            }
            ...
            $_SESSION['message'] = 'An unexpected error occurred while adding the schedule.';
            $_SESSION['message_type'] = 'danger';
        }
    } else {
