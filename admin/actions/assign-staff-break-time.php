<?php
// actions/assign-staff-break-time.php
session_start();
include('../../db_conn.php');
...
// Check if staff_id and break_time_id are set in the POST request
if (isset($_POST['staff_id']) && isset($_POST['break_time_id'])) {
    $staff_id = sanitize($_POST['staff_id'], $conn);
    ...
    if ($stmtCheck) {
        ...
        if (mysqli_stmt_num_rows($stmtCheck) === 1) {
            ...
            // Update the staff_account table to set the break_time_id
            $updateQuery = "UPDATE staff_account SET break_time_id = ? WHERE staff_id = ?";
            $stmtUpdate = mysqli_prepare($conn, $updateQuery);

            if ($stmtUpdate) {
                // Bind parameters
                mysqli_stmt_bind_param($stmtUpdate, "ii", $break_time_id, $staff_id);
    ...
    echo json_encode([
        'success' => false,
        'message' => "Invalid request. Staff ID and Break Time are required."
    ]);
}
?>
