...
<body>
    ...
    <div class="breadcrumbs-container">
        <nav aria-label="breadcrumb" class="breadcrumbs ms-4 bg-white border shadow rounded py-2 px-3">
            <ol class="breadcrumb mb-0">
                ...
                <li class="breadcrumb-item active text-muted" aria-current="page">Faculty DTR</li>
            </ol>
        </nav>
    </div>
    
    <p class="classTitle fs-4 fw-bold ms-5 mb-2">Faculty DTR</p>
    ...
    <div class="container bg-white rounded border shadow mb-5 p-4">
        ...
        <div class="d-flex justify-content-end mb-3">
            <button id="batchPrintBtn" class="btn custom-dark-brown-btn me-3" onclick="window.location.href='breaktime.php'">
                <i class="bi bi-cup-hot"></i> Break Time
            </button>
            <button id="batchPrintBtn" class="btn custom-dark-brown-btn" onclick="window.location.href='batch-faculty-dtr.php'">
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-striped">
                ...
                <tbody id="tableBody">
                    <?php
                    // Fetch teacher data with break time details
                    $query = "SELECT ta.teacher_id, ta.firstName, ta.middleName, ta.lastName, ta.department, ta.emp_code, ta.status, 
                                     ta.break_time_id, bt.break_name, bt.start_break, bt.end_break
                              FROM teacher_account ta
                              LEFT JOIN break_time_schedule bt ON ta.break_time_id = bt.break_time_id
                              ORDER BY FIELD(ta.status, 'Part-timer', 'Regular'), ta.lastName ASC";
                    $result = mysqli_query($conn, $query);
                    if (!$result) {
                        echo "<tr><td colspan='7' class='text-center text-danger'>Error fetching data: " . mysqli_error($conn) . "</td></tr>";
                    } else {
                        $index = 1;
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            $employeeID = !empty($row['emp_code']) 
                                ...
                                : '<span class="text-muted"><i>Assign the Employee ID to match the teacher\'s ID in the biometric device in Teacher List page.</i></span>';
                            $status = htmlspecialchars($row['status']);

                            // Determine Break Time display
                            if ($status === 'Part-timer') {
                                $breakTimeDisplay = '<span class="text-muted">Break time managed via Schedule button.</span>';
                            ...
                            ?>
                            <tr id="row-<?= htmlspecialchars($row['teacher_id']); ?>">
                                <td><?= $index++; ?></td>
                                <td><?= $fullName; ?></td>
                                <td><?= htmlspecialchars($row['department']); ?></td>
                                <td><?= $employeeID; ?></td>
                                <td><?= $status; ?></td>
                                <td id="break-time-<?= htmlspecialchars($row['teacher_id']); ?>" data-break_time_id="<?= htmlspecialchars($row['break_time_id']); ?>">
                                    <?= $breakTimeDisplay; ?>
                                </td>
                                <td class="text-center">
                                    <button class="btn btn-outline-custom btn-sm" style="height:26px;" onclick="redirectToSingleDTR(<?= htmlspecialchars($row['teacher_id']); ?>)">
                                        <i class="bi bi-printer print-icon"></i> Print DTR
                                    </button>
                                    <?php if ($status === 'Regular'): ?>
                                        <button class="btn btn-outline-custom btn-sm me-2" 
                                                onclick="showBreakTimeModal(<?= htmlspecialchars($row['teacher_id']); ?>)">
                                            <i class="bi bi-cup-hot"></i> Break Time
                                        </button>
                                    <?php endif; ?>
                                    <?php if ($status === 'Part-timer'): ?>
                                        <a href="part-timer-faculty.php?teacher_id=<?= htmlspecialchars($row['teacher_id']); ?>" class="btn btn-outline-custom btn-sm me-2">
            ...
            <form id="assignBreakTimeForm">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="assignBreakTimeModalLabel">Assign Faculty Break Time</h5>
    ...
    <script>
        ...
        function showBreakTimeModal(teacherId) {
            // Set the teacher ID in the hidden input field within the modal
            document.getElementById('modalTeacherId').value = teacherId;

            // Retrieve the current break_time_id from the table
            const breakTimeCell = document.getElementById(`break-time-${teacherId}`);
        ...
        document.getElementById('assignBreakTimeForm').addEventListener('submit', function(event) {
            ...
            fetch('actions/assign-breaktime.php', {
                method: 'POST',
                body: formData,
